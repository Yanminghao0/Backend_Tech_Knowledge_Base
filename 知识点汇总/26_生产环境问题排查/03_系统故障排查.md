...
# Azure VMSS扩容
az vmss scale --name my-vmss --resource-group my-rg --new-capacity 5
```
...
public String paymentServiceFallback(PaymentRequest request, Exception e) {
    log.warn("Payment service is down, using fallback: {}", e.getMessage());
    // 返回默认值或使用缓存数据
    return "payment_fallback_" + UUID.randomUUID().toString();
}
```
