...
echo "堆转储文件已生成: heap_${PID}_*.hprof"
```
...
echo "=== 平均响应时间 ==="
awk '{sum+=$10; count++} END {print "平均响应时间:", sum/count, "ms"}' $LOG_FILE
```
...
# 查看镜像构建历史
docker history <image_id/tag>
# 保存镜像到文件
docker save -o <file_name.tar> <image_id/tag>
# 加载镜像从文件
...
# 测试Pod间网络连通性
kubectl exec -it <pod_name> -n <namespace> -- ping <target_ip>
# 查看Pod IP和Service对应关系
...
# 查看Pod挂载情况
kubectl describe pod <pod_name> -n <namespace> | grep Volumes -A 30
```
