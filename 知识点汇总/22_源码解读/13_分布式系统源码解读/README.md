...
│  阶段1: 发现（Discovery）                               │
│  ┌──────────────────────────────────────────┐        │
│  │  Follower → 推荐自己(Looking状态)         │        │
│  │  交换选票: (myid, zxid)                    │        │
│  │  过半机制选出Leader(zxid最大优先)          │        │
│  │  新Leader生成新epoch                       │        │
│  └──────────────────────────────────────────┘        │
│                                                        │
│  阶段2: 同步（Synchronization）                        │
│  ┌──────────────────────────────────────────┐        │
│  │  Leader → Follower发送DIFF/TRUNC/DIFF      │        │
│  │  Follower同步到Leader的zxid                │        │
│  │  确保所有Follower数据一致                  │        │
│  └──────────────────────────────────────────┘        │
│                                                        │
│  阶段3: 广播（Broadcast）                               │
│  ┌──────────────────────────────────────────┐        │
│  │  Leader收到写请求                          │        │
│  │  → 分配新ZXID (epoch++ + counter)         │        │
│  │  → Proposal发送给所有Follower              │        │
│  │  → Follower ACK                            │        │
│  │  → 过半ACK → Commit → 通知Follower Commit  │        │
│  └──────────────────────────────────────────┘        │
│                                                        │
│  ZXID结构:                                            │
│  ┌────────────────┬────────────────────────┐         │
...
│  ┌──────────────────────────────────────────┐        │
│  │  1. 节点初始为Follower                    │        │
│  │  2. 选举超时(150-300ms随机) → 变为Candidate│        │
│  │  3. 自增Term，投自己一票                   │        │
│  │  4. 发送RequestVote给其他节点              │        │
│  │  5. 过半同意 → 成为Leader                  │        │
│  │  6. 随机超时防止多候选人同时选举            │        │
│  └──────────────────────────────────────────┘        │
│                                                        │
...
│  │  Leader:                                  │        │
│  │    1. 写入本地日志 (uncommitted)           │        │
│  │    2. 发送AppendEntries给所有Follower      │        │
│  │    3. Follower写入本地日志，ACK            │        │
│  │    4. 过半ACK → Commit                    │        │
│  │    5. 应用到状态机 (BoltDB)               │        │
│  │    6. 返回客户端成功                       │        │
│  └──────────────────────────────────────────┘        │
...
│  ┌──────────────────────────────────────────┐        │
│  │  日志增长到阈值 → 触发Snapshot             │        │
│  │  1. 将已committed的日志应用到状态机        │        │
│  │  2. 生成Snapshot文件                      │        │
│  │  3. 删除Snapshot之前的日志条目             │        │
│  │  4. 新Follower可从Snapshot + 后续日志恢复  │        │
│  └──────────────────────────────────────────┘        │
└──────────────────────────────────────────────────────┘
```
...
│  ┌─────────────────────────────────────────┐          │
│  │  健康检查                                │          │
│  │  - HTTP检查 / TCP检查 / Script检查       │          │
│  │  - TTL检查 (服务主动上报)                │          │
│  │  - 检查结果通过Gossip传播                │          │
│  └─────────────────────────────────────────┘          │
│                                                        │
│  多数据中心: WAN Gossip Pool 连接各DC的Consul Server    │
└──────────────────────────────────────────────────────┘
```
...
- Etcd的Compact机制？不及时Compact会有什么问题？
- Raft的网络分区处理？（少数派分区不能选举，多数派分区正常服务）
```
...
| 通信协议 | 自定义TCP | gRPC | HTTP/gRPC |
| 事务 | 乐观锁(version) | Mini事务(Txn) | Check-and-Set |
| 历史版本 | 不支持 | MVCC多版本 | 不支持 |

---

## 🔗 配套知识点

- [05_分布式理论/CAP与BASE理论](../../05_分布式理论/) — 分布式理论基础
- [03_数据库/分布式事务](../../03_数据库/) — 分布式一致性实践
- [06_微服务/服务注册中心](../../06_微服务/) — ZK/Etcd/Consul在注册中心的应用
