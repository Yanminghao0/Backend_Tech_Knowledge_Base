...
### 3. 高并发性能调优案例集 ⭐⭐⭐
📄 [高并发性能调优案例集.md](./03_高并发性能调优案例集.md)

**核心内容**：
- ✅ **实战案例**：多种高并发场景的性能调优全过程

### 4. 数据库性能优化 ⭐⭐⭐⭐ 必学
📄 [数据库性能优化.md](./04_数据库性能优化.md)

**核心内容**：
- ✅ **慢SQL定位**：慢查询日志、EXPLAIN执行计划分析
- ✅ **索引优化**：覆盖索引、最左前缀、索引下推ICP、索引失效场景
- ✅ **SQL改写**：JOIN vs子查询、深度分页优化、批量操作
- ✅ **连接池调优**：HikariCP配置、连接池大小计算公式
- ✅ **MySQL参数**：innodb_buffer_pool_size、刷盘策略、内存分配

### 5. 接口性能优化实战 ⭐⭐⭐⭐ 必学
📄 [接口性能优化实战.md](./05_接口性能优化实战.md)

**核心内容**：
- ✅ **性能指标**：QPS/RT/P99分位值、并发数计算
- ✅ **瓶颈定位**：Arthas trace、JProfiler、瓶颈决策树
- ✅ **串行转并行**：CompletableFuture、自定义线程池
- ✅ **异步化**：@Async、MQ异步解耦
- ✅ **缓存策略**：多级缓存、穿透/击穿/雪崩防护
- ✅ **实战案例**：接口从2000ms优化到50ms全过程

### 6. 压测与性能分析 ⭐⭐⭐⭐
📄 [压测与性能分析.md](./06_压测与性能分析.md)

**核心内容**：
- ✅ **工具对比**：JMeter/wrk/gatling/k6选型
- ✅ **JMeter实战**：CLI模式、动态参数化、断言
- ✅ **wrk指南**：Lua脚本扩展、动态参数
- ✅ **性能指标**：P50/P90/P99/P999分位值解读
- ✅ **压测策略**：阶梯加压、恒压测试、脉冲测试
- ✅ **瓶颈分析**：CPU/内存/IO/网络排查命令
- ✅ **实战案例**：秒杀系统压测全流程
