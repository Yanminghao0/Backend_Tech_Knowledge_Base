...
vLLM — 生产级LLM推理引擎
  ...
  │                                                  │
  │  性能对比(7B模型, A100):                          │
  │  Ollama: ~200-500 tokens/s (单请求, GPU)         │
  │  vLLM:  ~2000+ tokens/s (多请求并发, A100)       │
  │  → vLLM并发吞吐远高于Ollama                      │
  └──────────────────────────────────────────────────┘
```
...
LLM推理显存 = 模型参数 + KV Cache + 激活值
  ...
  KV Cache(每个请求):
    ...
    Qwen2.5-7B (GQA, n_kv_heads=4):
      2 × 28 × 4 × 128 × 4096 × 2 = 0.22GB/请求(4K上下文)
      10个并发 → 2.2GB

  总显存估算:
    7B FP16 + 10并发4K = 14 + 2.2 ≈ 16GB → RTX 4090(24GB)够
    7B INT4 + 10并发4K = 3.5 + 2.2 ≈ 6GB → RTX 3090(24GB)绰绰有余
    70B INT4 + 10并发4K = 35 + 12.5 ≈ 48GB → A100(80GB)够
```
